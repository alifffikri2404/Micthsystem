...
if (isset($_POST["update"])) {
    ...
    header("Location: editHRform.php");
    exit();
}
...
if (isset($_POST['fun1'])) {
    $uid = $_POST['uid'];
    $sql = "UPDATE empmaininfo SET function_hr1 = '1' WHERE First_Name = '$uid'";
    if ($conn->query($sql)) {
        echo "<script>alert('Role successfully switched.')</script>";
    } else {
        echo "<script>alert('Role failed to activate. " . mysqli_error($conn) . "')</script>";
    }
}

if (isset($_POST['fun2'])) {
    $uid = $_POST['uid'];
    $sql = "UPDATE empmaininfo SET function_hr1 = '0'  WHERE First_Name = '$uid'";
    if ($conn->query($sql)) {
        echo "<script>alert('Role successfully switched.')</script>";
    } else {
        echo "<script>alert('Role failed to activate. " . mysqli_error($conn) . "')</script>";
...
<body>
  ...
  <li class="nav-item">
    ...
    <main id="main" class="main">
        ...
        <div class="pagetitle">
            <h1>Human Resources & Admin Staff</h1>
        </div><!-- End Page Title -->
            ...
            <tbody>
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    if ($row['Active_Inactive'] == 'Active') {
                        ...
                        echo "<button type='submit' class='btn btn-success' name='Active'>Activate</button>";
                    }
                    
                    echo "&nbsp;<button class='btn btn-danger' type='submit' name='submit' onclick='return checkdelete();'>Delete</button>
                            </form>
                        </td>
                    </tr>";
                }

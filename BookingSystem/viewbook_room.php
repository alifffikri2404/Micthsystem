...
WHERE Internal_Id  = '$userid'";
                            ...
                            <div class="col-lg-7">
                              ...
                              <div class="row g-3">
                                <div class="col-md-12">
                                  ...
                                  <textarea id="vehicle" name="remark12" class="form-control" style="height: 150px;"></textarea>
                                </div>
                              </div>
                              <br>
                              <div class="row">
      ...
      if (isset($_POST['takekey'])) {
        ...
        $appUpdateQuery = "UPDATE management_room SET status = ?, Approver = ? WHERE id = ?";

        $stmt1 = $db->prepare($appUpdateQuery);
        
        $status13 = 'Reserved';
        $stmt1->bind_param('ssi', $status13, $Approver, $_POST['row_id']);
...
Thank you!
      ...
      if (isset($_POST['reject'])) {
        if (isset($_POST['row_id'])) {
          ...
          $appUpdateQuery = "UPDATE management_room SET status = 'Reject', rejected='$rejected' WHERE id = '$row_id'";
          $appUpdateResult = mysqli_query($db, $appUpdateQuery);

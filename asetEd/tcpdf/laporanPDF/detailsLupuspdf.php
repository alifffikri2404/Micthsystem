...
// 2nd query
$sqlB= "SELECT * FROM disposal_aset
WHERE aset_id = $idaset";

$rowB = mysqli_query($conn2, $sqlB);

if (!$rowB) {
    die("Error: " . mysqli_error($conn2));
}

$rowBB = mysqli_fetch_array($rowB);
$tarikh_lupus = isset($rowBB['tarikh_lupus']) ? $rowBB['tarikh_lupus'] : '';
$nilai_lupus = isset($rowBB['nilai_lupus']) ? $rowBB['nilai_lupus'] : '';
$pihak_lupus = isset($rowBB['pihak_lupus']) ? $rowBB['pihak_lupus'] : '';

$reason_lupus = isset($rowBB['reason']) ? $rowBB['reason'] : '';
$reason_lupus2 = '';
if (!empty($reason_lupus) && $reason_lupus != '-') {
	$sqlA4 = "SELECT * FROM disposal_reason WHERE id_reason = $reason_lupus";
	$result4 = mysqli_query($conn2, $sqlA4);
	if ($result4 && mysqli_num_rows($result4) > 0) {
		$rowA4 = mysqli_fetch_assoc($result4);
		$reason_lupus2 = isset($rowA4['reason']) ? $rowA4['reason'] : 'None';
	} else {
	  $reason_lupus2 = "None";
	}
} else {
...
$pdf->SetTitle('Asset Disposal Report');
$pdf->SetSubject('SDM');
...
$pdf->Write(0, 'Disposal Details', '', 0, 'L', true, 0, false, false, 0);
...
$pdf->writeHTML($html, true, false, true, false, '');
        ...
        <tr>
			<td align="left" colspan="2" style="font-size:13;"><strong>Disposal Information</strong> </td>			
		</tr>
		<tr>
			<td align="left"><strong>Disposal Date</strong> : '.$tarikh_lupus.'</td>
			<td align="left"><strong>Disposal Value</strong> : '.'RM '.$nilai_lupus.'</td>			
		</tr>
		<tr>
			<td align="left"><strong>Person-in-Charge</strong> : '.$pihak_lupus.'</td>
			<td align="left"><strong>Reason for Dispose</strong> : '.$reason_lupus2.'</td>			
		</tr>
		';

    ...
    <div class="row">
      ...
      <div class="col-12">
        <div class="card top-selling">
          <!-- <div class="sb"> -->
            <div class="card-body">
              <h5 class="card-title"><strong>ASSET LOG HISTORY</strong><br/>
                    ...
                    <div class="row">
                      <div class="col-md-5">
                        <div class="form-group" style="display: flex; align-items: center;">
                          <?php
                            $sqlvtp2 = "SELECT * FROM tbl_daftar_aset
                              WHERE tbl_daftar_aset.id =" . $_GET['id'];
                            $result12 = mysqli_query($conn2, $sqlvtp2);
                            $rowAAA = mysqli_fetch_array($result12);

                            $namaaset = ucwords($rowAAA['nama_aset']);
                          ?>
                          <label for="logaset" style="font-weight: 400; 'Nunito', sans-serif;">Asset Name:</label>
                          <h5 style="font-weight: 500; font-size: 20px; font-family: 'Poppins', sans-serif; margin-left: 10px"><strong><?php echo $namaaset; ?></strong></h5>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div class="row">
                ...
                <div class="box-header with-border">
                  <div class="col-md-12">
                    <!-- /.box-header -->
                    <div class="box-body">
                      <form action="../../tcpdf/laporanPDF/logpdf.php" name="form2" method="post" target="_blank">
                        <?php
                        //$sqlAP = mysql_query("SELECT * FROM jenis_aset ORDER BY id ASC");
                        $sqlA_query = "SELECT DISTINCT tbl_daftar_aset.* FROM tbl_daftar_aset 
                        WHERE tbl_daftar_aset.id=" . $_GET['id'];
                        $result_set = mysqli_query($conn2, $sqlA_query);
                        $fetched_row = mysqli_fetch_array($result_set);
                  
                        $sqlAP = "SELECT DISTINCT log_aset.*
                        FROM log_aset
                        INNER JOIN tbl_daftar_aset ON tbl_daftar_aset.id = log_aset.aset_id
                        WHERE tbl_daftar_aset.id = ". $_GET['id'];
                        $result3 = mysqli_query($conn2, $sqlAP);
                        $countAP = mysqli_num_rows($result3);
                          ...
                          <table id="example1" class="table table-bordered table-striped">
                            <thead>
                              <tr>
                                ...
                                <th style="width: 40px">Registration Date</th>
                                <th style="width: 40px">Handover Date</th>  
                            ...
                            <tbody>
                              <?php
                                $off = 0;
                                $i = 1 + $off;
                                while($rowAP = mysqli_fetch_array($result3)) {
                                  $lokasi_jabatan = $rowAP['lokasi_jabatan'];
                                  if (!empty($lokasi_jabatan) && ($lokasi_jabatan != '-')){
                                  ...
                                  echo '
                                    ...
                                    <td data-title="Registration Date">'.strtoupper($rowAP['tarikh_daftar']).'</td>
                                    <td data-title="Handover Date">'.strtoupper($rowAP['tarikh_serahan']).'</td>
                                    <td data-title="Asset Number">'.strtoupper($rowAP['no_aset']).'</td>
                                    <td data-title="Asset Name">'.strtoupper($rowAP['nama_aset']).'</td>	
                                    <td data-title="Staff Name">'.strtoupper($rowAP['nama_kakitangan']).'</td>
                                    <td data-title="Department">'.strtoupper($lokasi_jabatan2).'</td>
                                  </tr>';
                                $i++;
                                }
                                ?>
                        ...
                        ?>
                        <td width="25%">
                          <a href="../../tcpdf/laporanPDF/logpdf.php?id=<?php echo $fetched_row['id']; ?>" target="_blank" class="btn btn-info btn-lg"

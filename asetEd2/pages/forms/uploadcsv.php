...
if (isset($_POST["submit"])) {
    // Debugging: Print out the $_FILES array to check its contents


    // Check if 'file' key exists in $_FILES array
        ...
        if (is_uploaded_file($file)) {
            ...
            if ($handle !== FALSE) {
                ...
                while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                    ...
                    $supplier = $data[11]; // Assuming SUPPLIER is the 12th column in your CSV

                    // Prepare the SQL query to prevent SQL injection
                    $stmt = $conn2->prepare("INSERT INTO asset_management_vba (Category, `Category ID`, `Sub-Category`, `Sub-Category ID`, Model, `Model ID`, `Running No`, `Full ID (Concatenated ID)`, Barcode, `SERIAL NO.`, `DATE OF PURCHASE`, `SUPPLIER`) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
<body>
    ...
    <div class="col-12">
        <div class="card top-selling">
            <div class="card-body">
                <h5 class="card-title"><strong>STAFF ASSET REQUEST</strong></h5>
                <form action="uploadcsv.php" method="post" enctype="multipart/form-data">
                    <label for="file">Choose CSV file:</label>
                    <input type="file" name="file" id="file" />
                    <br>
                ...
                <form role="form" action="" method="post">
                                    ...
                                    <select class="form-select" id="kategori" name="kategori" 
                                        ...
                                        if ($result) {
                                            while ($rowL = mysqli_fetch_array($result)) {
                                                ...
                                                echo  '<option value="' . $rowL['id_kategori'] . '" ' . $selected . '>' . $rowL['nama_kategori'] . ' ' . $rowL['id_kategori'] . '</option>';
                                            }
                                        }
                                        ?>
                                    </select>
                    ...
                    <div class="col-md-2">
                        <div class="form-group">
                            ...
                            <button type="reset" name="reset" id="reset" class="btn btn-danger btn-lg" style="font-size: 15px">Reset</button>
                        </div>
                    </div>
                </form>
...
if (isset($_POST['submit1'])) {
    ...
    $kategori = str_pad($kategori, 2, '0', STR_PAD_LEFT);

    $sqlAA = "SELECT * FROM asset_management_vba WHERE `Category ID` = '$kategori'";
    ...
    if (mysqli_num_rows($resultAA) > 0) {
        ...
        while ($rowBB = mysqli_fetch_assoc($resultAA)) {
            ...
            echo '<td>' . htmlspecialchars($rowBB['Sub-Category']) . '</td>';
            echo '<td>' . htmlspecialchars($rowBB['Model']) . '</td>';
            echo '<td>' . htmlspecialchars($rowBB['Full ID (Concatenated ID)']) . '</td>';
